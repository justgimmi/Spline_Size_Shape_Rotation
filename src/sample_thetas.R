...
sample_theta_joint <- function(init, hyper, X, burn = TRUE) {
  k_l <- init$k_l
  hyper$t <- hyper$t + 1
  
  # 1. Estrazione dei gap attuali (delta)
  delta_curr <- numeric(k_l)
  delta_curr[1] <- init$thetas[1]
  if (k_l > 1) {
    for (l in 2:k_l) {
      delta_curr[l] <- init$thetas[l] - init$thetas[l-1]
    }
  }
  
  # 2. Generazione della proposta joint per tutti i gap
  proposal_noise <- mvrnorm(n = 1, 
                            mu = rep(0, k_l), 
                            Sigma = hyper$lambda_theta * hyper$Sigma_theta)
  delta_cand <- delta_curr + proposal_noise
  
  # Variabili di controllo per verificare se valutare o meno la verosimiglianza
  constraints_failed <- any(delta_cand <= 1e-6) || sum(delta_cand) >= (2 * pi - 1e-6)
  
  accepted <- FALSE
  log_accept_ratio <- -Inf
  
  if (!constraints_failed) {
    # 4. Ricostruzione dei candidati theta se i vincoli sono ok
    theta_cand <- cumsum(delta_cand)
    
    # 5. Valutazione della verosimiglianza
    res_cand <- eval_log_lik_theta_joint(theta_cand, init, hyper, X)
    
    # 6. Calcolo del ratio di accettazione Metropolis-Hastings
    log_accept_ratio <- min(res_cand$log_lik - hyper$log_lik, 0)
    
    if (log(runif(1)) < log_accept_ratio) {
      accepted <- TRUE
      
      # Aggiornamento dello stato (Accettato)
  ...
  # 7. Fase di Adattamento Unificata (Eseguita AD OGNI iterazione se burn == TRUE)
  if (burn == TRUE) {
    hyper$gamma_theta <- min(0.01, hyper$t^(-0.5))
    
    # Se i vincoli sono falliti o rifiutati, la probabilità di accettazione empirica è 0 (exp(-Inf) = 0)
    alpha_emp <- if (constraints_failed) 0 else exp(log_accept_ratio)
    hyper$lambda_theta <- hyper$lambda_theta * exp(hyper$gamma_theta * (alpha_emp - hyper$theta_opt))
    
    # Se accettato adatta sul candidato, altrimenti adatta sul valore corrente (delta_curr)
    delta_for_adapt <- if (accepted) delta_cand else delta_curr
    
    hyper$Sigma_theta <- hyper$Sigma_theta + hyper$gamma_theta * ((delta_for_adapt - hyper$mu_theta) %*% t(delta_for_adapt - hyper$mu_theta) - hyper$Sigma_theta)
    hyper$mu_theta    <- hyper$mu_theta + hyper$gamma_theta * (delta_for_adapt - hyper$mu_theta)
  }
  
  #print(if (constraints_failed) 0 else exp(log_accept_ratio))
}
...
sample_theta_joint_dirichlet <- function(init, hyper, X) {
  k_l <- init$k_l
  
  # 1. Extract current gaps (including the remainder gap to hit exactly 2*pi)
  delta_curr <- numeric(k_l + 1)
  delta_curr[1] <- init$thetas[1]
  for (l in 2:k_l) {
      delta_curr[l] <- init$thetas[l] - init$thetas[l-1]
  }
  delta_curr[k_l + 1] <- 2 * pi - init$thetas[k_l]
  
  # Normalize to a true simplex (sums to 1) for the Dirichlet distribution
  simplex_curr <- delta_curr / (2 * pi)
  
  # 2. Set Dirichlet concentration parameters centered on current state
  # hyper$kappa_theta acts like inverse variance (e.g., start around 1000 or 5000)
  alpha_forward <- simplex_curr * 500
  
  # 3. Generate candidate simplex and scale back to gaps
  simplex_cand <- as.numeric(rdirichlet(1, alpha_forward))
  
  # Safeguard against boundary underflows
  #if (any(simplex_cand < 1e-6)) return(init)
  
  delta_cand <- simplex_cand * (2 * pi)
  
  # 4. Reconstruct candidate thetas (drop the remainder gap for the actual thetas)
  theta_cand <- cumsum(delta_cand[1:k_l])
  
  # 5. Evaluate full joint likelihood
  res_cand <- eval_log_lik_theta_joint(theta_cand, init, hyper, X)
  
  # If likelihood is completely invalid, reject immediately
  if (res_cand$log_lik == -Inf){return()}
  
  # 6. Compute Asymmetric Proposal Densities (Transition probabilities)
  # Forward proposal density: q(simplex_cand | simplex_curr)
  log_q_forward <- log(ddirichlet(simplex_cand, alpha_forward))
  
  # Backward proposal density: q(simplex_curr | simplex_cand)
  alpha_backward <- simplex_cand * 500
  log_q_backward <- log(ddirichlet(simplex_curr, alpha_backward))
  
  # 7. Metropolis-Hastings Acceptance Step
  # Formula: (Likelihood Ratio) * (Backward Proposal / Forward Proposal)
  log_accept_ratio <- (res_cand$log_lik - hyper$log_lik) + (log_q_backward - log_q_forward)
  
  if (log(runif(1)) < log_accept_ratio) {
    print("accepted")
    # Accept changes and update environment states
    init$thetas   <- theta_cand
    init$mean     <- res_cand$mean
    init$mean_i   <- res_cand$mean_i
    init$residual <- res_cand$residual
